...
title GynoCare AI - REPAIR AND RECOVERY
...
echo.
echo  ================================================================
echo   GynoCare AI  -  REPAIR AND RECOVERY TOOL
echo  ================================================================
...
:PORT
echo.
echo  Fixing port 5055...
netstat -ano 2>nul | findstr ":5055 " >nul 2>&1
if %errorlevel%==0 (
    for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5055 "') do (
        echo   Stopping PID %%P...
        taskkill /PID %%P /F >nul 2>&1
    )
    echo   [ OK ]  Port 5055 freed.
) else (
    echo   [ OK ]  Port 5055 already free.
)
goto :DONE
...
netstat -ano 2>nul | findstr ":5055 " >nul 2>&1
if %errorlevel%==0 (for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5055 "') do taskkill /PID %%P /F >nul 2>&1)
echo   [ OK ]  Port done.
if exist "%VENV_DIR%" rmdir /s /q "%VENV_DIR%" >> "%LOGFILE%" 2>&1
"%PYTHON_EXE%" -m venv "%VENV_DIR%" >> "%LOGFILE%" 2>&1
...
echo  ================================================================
echo.
echo   S = Start GynoCare AI   D = Run Diagnostic   X = Exit
echo.
set /p "POST=  Choice [S/D/X]: "
if /i "%POST%"=="S" call "%BASEDIR%\START_GynoCare_AI.bat"
